 ...
 *
 * Vérifié par watches.test.ts, qui remplace `fetch` et inspecte l'appel réel.
 *
 * Indice : calquez le pattern de `fetchWatches` ci-dessus, juste avec le bon path.
 */
 ...
 *   échec  → lit le body d'erreur (`{ error: "..." }`) et throw un Error avec ce message
 *           si possible, sinon "Update failed (HTTP <status>)".
 *
 * Vérifié par watches.test.ts, qui inspecte la méthode HTTP, l'en-tête
 * Content-Type, le body envoyé, et les deux cas d'erreur.
 *
 * Indice : calquez le pattern de `purchaseWatch` ci-dessus, mais en `PATCH` au
 * lieu de `POST`.
 *
